...
import com.test.diff.services.repository.RepoInfoRepository;
...
public class DiffWorkFlow {
	...
	@Resource
	private RepoInfoRepository repoInfoService;
	...
	public List<ClassInfo> diff(ProjectDiffParams params) throws GitAPIException, IOException {
		...
		// 获取base repository
		String projectGroup = params.getProjectGroup();
		String env = params.getEnv();
		String projectName = params.getProjectName();
		String projectUrl = params.getProjectUrl();
		Long repoId = params.getRepoId();
		BaseRepository baseRepository = initRepository(repoId);

		Git baseGit, newGit;
		AbstractTreeIterator baseTree, newTree;
		switch (params.getDiffTypeCode()) {
		case BRANCH_DIFF:
			baseGit = syncCode(baseRepository, projectGroup, env, projectName, projectUrl, params.getOldVersion(), "",
					true);
			newGit = syncCode(baseRepository, projectGroup, env, projectName, projectUrl, params.getNewVersion(), "",
					true);
			...
			break;
		case COMMIT_DIFF:
			baseGit = syncCode(baseRepository, projectGroup, env, projectName, projectUrl, params.getNewVersion(),
		...
		List<CompletableFuture<ClassInfo>> futures = list.stream().map(diffEntry -> diffHandle(diffEntry,
				getFilePath(baseGit, diffEntry.getOldPath()), getFilePath(newGit, diffEntry.getNewPath())))
				.collect(Collectors.toList());
		CompletableFuture.allOf(futures.toArray(new CompletableFuture[0])).join();

		List<ClassInfo> classInfos = futures.stream().map(CompletableFuture::join).filter(Objects::nonNull)
				.collect(Collectors.toList());
	...
	private String getFilePath(Git git, String packName) {
		return git.getRepository().getDirectory().getAbsolutePath().replace(GitConst.GIT_FILE_SUFFIX, "") + packName;
	}
	 ...
	 */
	private List<DiffEntry> getDiffCodeClasses(Git newGit, AbstractTreeIterator baseTree, AbstractTreeIterator newTree)
			throws GitAPIException {
		List<DiffEntry> list;
...
//                .filter(diffEntry -> diffEntry.getNewPath().contains(GitConst.JAVA_DEFAULT_PATH))
	...
	private Git syncCode(BaseRepository baseRepository, String projectGroup, String env, String projectName,
			String projectUrl, String branch, String commitId, boolean isBranchDiff) throws IOException {
		// 这里直接生成路径
		String branch_path = fileUtil.getRepoPath(projectGroup, env, projectName, branch);
		if (!isBranchDiff) {
			branch_path = branch_path + "_" + commitId;
		}
		return updateCode(baseRepository, branch_path, projectName, projectUrl, branch, commitId, isBranchDiff);
	}

	/**
	 * 暂时不用了,这里是直接从code-diff根目录开始建项目文件夹的
	 * 后期都改成了由group/env/project，这里就不适用了，没有传group和env，所以直接不用
	 * 改为上游方法中直接调用fileUtil类中方法直接生成path 检查项目路径是否正确
	 * 
	 * @param path        项目路径
	 ...
	 */
	@Deprecated
	private String checkProjectPath(String path, String projectName, String branch, String commitId,
			boolean isBranchDiff) {
	...
	private Git updateCode(BaseRepository repository, String path, String projectName, String projectUrl, String branch,
			String commitId, boolean isBranchDiff) throws IOException {
